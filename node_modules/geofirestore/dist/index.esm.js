...
function encodeSetDocument(data, options) {
    if (Object.prototype.toString.call(data) === '[object Object]') {
        ...
        const unparsed = 'd' in data ? data.d : data;
...
function findCoordinates(document, customKey, flag = false) {
    let error;
    let coordinates;
    if (!customKey) {
...
function validateGeoDocument(data, flag = false) {
    let error;
...
class GeoDocumentReference {
    ...
    get onSnapshot() {
        return (onNext, onError) => {
            return this
                ._document.onSnapshot(snapshot => onNext(new GeoDocumentSnapshot(snapshot)), error => {
                if (onError) {
                    onError(error);
                }
            });
    ...
    constructor(snapshots, _queryCriteria) {
        ...
        if (this._queryCriteria.limit &&
            ...
            const arrayToLimit = Array.from(this._docs.values())
                .map(doc => {
                return {
                    distance: calculateDistance(this._queryCriteria.center, doc.data().l),
                    id: doc.id,
                };
            })
